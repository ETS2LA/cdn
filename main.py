...
async def return_translations():
    if translations.IsAvailable():
        ...
        return StreamingResponse(LimitedStreamer(FilePath=file_path, ChunkSize=1024, SPEEDLIMIT), media_type="application/octet-stream", headers={"content-length": content_length})
    else:
