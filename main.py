...
async def return_translations():
    if translations.IsAvailable():
        ...
        return StreamingResponse(LimitedStreamer(FilePath=file_path, ChunkSize=16384, SpeedLimitKbps=SPEEDLIMIT), media_type="application/octet-stream", headers={"content-length": content_length})
    else:
